...
from google3.third_party.nucleus.util import sequence_utils
...
def reverse_complement(sequence: str) -> str:
  ...
  google3/third_party/nucleus/util/sequence_utils.py
...
def deepconsensus_input_to_example(
  ...
  for i in range(min(len(deepconsensus_input.subreads), max_passes)):
    ...
    encoded_bases = encode_dna_as_floats(subread.bases)
  ...
  if deepconsensus_input.ccs_sequence:
    encoded_ccs_bases = encode_dna_as_floats(deepconsensus_input.ccs_sequence)
  ...
  label_bases_list = encode_dna_as_floats(deepconsensus_input.label.bases)
