...
class ParseTfExamplesDoFn(beam.DoFn):
  ...
  def process(
    ...
    example_info = data_providers.process_input(
        proto_string=proto_string, params=self.params)
...
class RunForwardPassDoFn(beam.DoFn):
  ...
  def setup(self):
    ...
    model_utils.modify_params(self.params)
  ...
  def process(
    ...
    record['edit_distance'] = edit_distance(dc_input.label.bases, y_pred_bases)
...
class CalcStratPosAccuracy(beam.DoFn):
  """Calculates accuracy by position."""

  def process(self, record: Dict[str, Any]) -> Iterable[Dict[str, Any]]:
    # Align label and prediction.
    label, prediction = colab_utils.align_sequences(record['label_bases'],
                                                    record['pred_bases'])
    label = label.replace('-', ' ')
    prediction = prediction.replace('-', ' ')
    matches = np.array([x == y for x, y in zip(label, prediction)])
    for pos, is_match in enumerate(matches):
      # Return pos (group) and stratified_pos_accuracy (group_val)
      yield {'pos': pos, 'stratified_pos_accuracy': float(is_match)}


class StratifiedPosAccToCSV(beam.PTransform):

  def __init__(self, out_dir: str):
    self.fname = 'stratified_pos_accuracy'
    self.out_dir = out_dir

  def expand(self, pcoll: Dict[str, Any]):
    _ = (
        pcoll
        | beam.ParDo(CalcStratPosAccuracy())
        | StatsToCsv(self.out_dir, 'pos', 'stratified_pos_accuracy'))


class GenerateErrorAnalysisDoFn(beam.DoFn):
  """Generates an error analysis ANSI output."""

  def __init__(self, group: str, params: ml_collections.ConfigDict):
    self.group = group
    self.params = params
    self.max_passes = self.params.max_passes

  def process(self, record: Dict[str, Any]):
    f = io.StringIO()
    base_indices, _, _, _, _, _ = tf_example_utils.get_indices(self.max_passes)
    subreads = [
        x.bases for x in record['dc_proto'].subreads[slice(*base_indices)]
    ]
    with contextlib.redirect_stdout(f):
      print(f'{self.group}={record[self.group]}\n\n')
      colab_utils.print_data_and_preds(
          subreads, record['label_bases'], record['pred_bases'],
          colab_utils.get_mv_pred(record['dc_proto']),
          record['dc_proto'].ccs_sequence)
      print('\n\n')
    yield f.getvalue()


class ErrorAnalysis(beam.PTransform):

  def __init__(self, out_dir: str, group: str,
  ...
  def expand(self, pcoll: Dict[str, Any]):
    _ = (
        pcoll
        | 'sample_examples' >> beam.Filter(lambda x: x['sample'])
        | 'add_group_key' >> beam.Map(lambda x: (x[self.group], x))
        | 'sample_by_key' >> beam.combiners.Sample.FixedSizePerKey(100)
        | 'fetch_records' >> beam.FlatMapTuple(lambda _, records: records)
        | 'generate_error_output' >> beam.ParDo(
            GenerateErrorAnalysisDoFn(group=self.group, params=self.params))
        | beam.io.textio.WriteToText(
            file_path_prefix=os.path.join(self.out_dir, 'error_analysis',
                                          f'{self.group}'),
            file_name_suffix='.ea.txt'))
