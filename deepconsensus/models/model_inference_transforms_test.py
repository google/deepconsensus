...
class StratifiedPosAccTest(absltest.TestCase):

  def test_per_position_accuracy(self):
    out_dir = self.create_tempdir().full_path
    data = [{
        'label_bases': 'CCCCCCCCCCTTTCCCCCCCCCC',
        'pred_bases': 'CCCCCCCCCCCCCCCCCCCC'
    }, {
        'label_bases': 'CCCCCCCCCCAAACCCCCCCCCC',
        'pred_bases': 'CCCCCCCCCCCCCCCCCCCC'
    }]

    expected = [1.0] * 10 + [0.0] * 3 + [1.0] * 10

    with test_pipeline.TestPipeline() as p:
      _ = (
          p
          | beam.Create(data)
          | model_inference_transforms.StratifiedPosAccToCSV(out_dir))

    # Read the output CSV, parse, and compare with expected values
    fname = 'stratified_pos_accuracy'
    output_path = os.path.join(out_dir, 'metrics', f'{fname}/{fname}__pos')
    df = pd.read_csv(output_path + '.stat.csv')
    self.assertTrue(
        np.equal(df.sort_values(by='group_val')['mean'].values, expected).all())


class ErrorAnalysisTest(absltest.TestCase):
  ...
  def test_error_analysis(self):
    ...
    group = 'num_passes'
    dc_input = test_utils.make_deepconsensus_input()
    # Introduce errors into the subreads
    dc_input.subreads[0].bases = 'ATCCA'
    out_dir = self.create_tempdir().full_path
    with test_pipeline.TestPipeline() as p:
      _ = (
          p
          ...
          | beam.ParDo(
              model_inference_transforms.RunForwardPassDoFn(
                  checkpoint_path=checkpoint_path, params=params))
          | model_inference_transforms.ErrorAnalysis(out_dir, group, params))
    ea_path = os.path.join(out_dir, 'error_analysis', f'{group}*')
    ea_file = open(glob.glob(ea_path)[0], 'r')
    self.assertEqual(ea_file.readlines()[0].strip(), 'num_passes=1')
