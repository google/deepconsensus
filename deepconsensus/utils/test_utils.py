...
from google3.third_party.nucleus.testing import test_utils
...
def deepconsensus_testdata(filename):
  ...
  Args:
    filename: The name of a testdata file in the core genomics testdata
      ...
      "learning/genomics/deepconsensus/testdata/foo.txt", filename should be
  ...
  return test_utils.genomics_testdata(
      os.path.join('deepconsensus/testdata', filename), 'learning/genomics')
...
def make_deepconsensus_input(**kwargs):
  ...
  return deepconsensus_pb2.DeepConsensusInput(
      subreads=subreads, label=label, **default_kwargs)
